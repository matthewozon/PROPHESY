...
if FLAG_CC
    ...
    peak_proba = P_CC;
    normalized_data = y_CC
    saveFileName = string(fileName,"_CC.xlsx")
else
    Ke = [mean(dictPeak[Symbol(string("hν_",toto))][!,Symbol("Kinetic energy")][3]) for toto in df_Eph[!,photon_sym]]
    peak_proba = P_COSO3;
    normalized_data = y_COSO3
    saveFileName = string(fileName,"_COSO3.xlsx")
end

dfDataAndMeta       = DataFrame("photon_energy"=>convert.(Float64,df_Eph[!,1]),"attenuation_length"=>λ_all,"kinetic_energy"=>Ke,
                        "total_cross_section"=>σ_C1s,"photon_flux"=>Fν,"transmition_value"=>T,"peak_proba"=>peak_proba,
...
dfProfile           = DataFrame("distance"=>r,"profile"=>ρC1s_bulk*ρ_cp)
dfCovMat            = DataFrame(["distance"=>r[N0:NB];[string("distance_",string(i,pad=3)) => Γρ_HI[i-N0+1,:] for i in N0:NB]])
dfHsharp            = DataFrame([Symbol("conc_profile_APE")=>ρC1s_bulk*ones(Cdouble,Nr);[Symbol(string("hν_",df_Eph[!,photon_sym][i])) => H_geom_sharp[i,:] for i in 1:Ndata]])
dfHsmooth           = DataFrame([Symbol("conc_profile_APE")=>ρC1s_bulk*MINOTAUR.f_logistic.(r,μ0,Δr_water;A=1.0);[Symbol(string("hν_",df_Eph[!,photon_sym][i])) => H_geom[i,:] for i in 1:Ndata]])
