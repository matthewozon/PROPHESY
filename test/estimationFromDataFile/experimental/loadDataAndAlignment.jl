...
for i in 1:Ndata
    ...
    y_peak_3[i] = dKe*sum(collect(skipmissing(dictAllData[plot_sym].Curve3))); # dKe*AePeak[3] -> COSO3-
    ...
    if FLAG_PLOT
        ...
        plot(Be,collect(skipmissing(dictAllData[plot_sym].Background.+dictAllData[plot_sym].Curve1.+dictAllData[plot_sym].Curve2.+dictAllData[plot_sym].Curve3)),label="fitted spectra curves") # C1s
        ylim(0.0)
    ...
    if FLAG_CC
        ...
        local λe = 1.0e-3dictPeak[plot_sym][!,:IMFP][3]
    end
    ...
    local ρ_sharp            = ρC1s_bulk*ones(Cdouble,Nr)
    local ρ                  = ρC1s_bulk*MINOTAUR.f_logistic.(r,μ0,Δr_water;A=1.0); # for the estimation of the alignement parameter, assume a concentration profile similar to that of water 
    α_al_noise_sharp[i],_    = noiseAndParameterEstimation(σ_all,H_geom_sharp[i,:],S_noisy,Sbg,ρ_sharp)
    α_al_noise_sharp[i]      = α_al_noise_sharp[i]/(κ_units*σ_C1s_exp(convert(Cdouble,df_Eph[!,photon_sym][i]))*dictPeak[plot_sym][!,ph_flu_sym][1])
